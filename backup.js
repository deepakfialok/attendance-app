...
function markStudentAttendance(classId, studentId, status, timing) {
  // Get the date from the date-picker or use the current date as a fallback
  ...
  if (confirm(`Mark ${data.students[studentId].name} as ${status}?`)) {
    // Add attendance record to local storage
    const attendanceRecord = {
      classID: classId,
      className: data.classes[classId].className,
      date: date, // Use the date from the date-picker or fallback
      time: timing,
    ...
    if (studentCard) {
      ...
      setTimeout(() => studentCard.remove(), 500); // Remove after animation completes
    }
  }
}
...
function loadClassWiseAttendance() {
  // Implement class-wise attendance analytics
  document.getElementById(
    "content"
...
function loadTeacherWiseAttendance() {
  // Implement teacher-wise attendance analytics
  document.getElementById(
    "content"
...
function loadStudentWiseAttendance() {
  // Implement student-wise attendance analytics
  document.getElementById(
    "content"
